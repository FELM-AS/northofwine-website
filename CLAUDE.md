...
| `_plugins/contentful_filters.rb` | Shared Liquid filters: `dir_href` (`"/<dir>/"`, the one shared place templates turn a resolved collection `dir` into a URL), `nb_number`/`country_lang` (Norwegian number formatting and the country→language lookup, Plan.md's Formatting/Hyphenation sections), and `unescape_html` (fixes a double-escape in `_includes/seo.html`'s meta-description fallback — see that file's own comment). |
...
A `contentful_data_collections` entry also accepts an optional `dir`, mirroring `contentful_collections`' `dir` — `EntriesGenerator` itself ignores it (a data collection still generates no per-entry pages), but `_plugins/contentful_listing_pages.rb` reads it as the single source of truth for that collection's own listing/filter pages (e.g. the manufacturer entry's `dir: produsenter` for `/produsenter/`).
...
- Sets `page.data["locale"]` to the locale code for every non-primary page (never for the primary locale, matching the `nil` "no locale in play" convention used everywhere else) — this is what `index.html`'s homepage listing uses to scope itself to one locale at a time via `where: "locale", page.locale`, a true no-op when every page's `locale` is `nil`. This relies specifically on **Jekyll's own** `where` filter override (`Jekyll::Filters#where`, via `compare_property_vs_target`'s explicit `when NilClass; return true if property.nil?`), not raw Liquid's `StandardFilters#where` (which has a different, truthy-only nil behavior) — the two are easy to conflate. Like `home_label`, a Contentful field literally named `locale` on some content type would silently collide and overwrite this (same class of risk noted below in "Field exposure" for `home_label`/body field names).
...
A manually-authored per-locale static page (e.g. a localized `nb-NO/index.html`, since `index.html` itself only ever renders the primary locale — see "Homepage & navigation") needs `locale: nb-NO` in its own front matter to match this convention; without it, `page.locale` is `nil` there too and the homepage `where` filter silently scopes it as if it were a primary-locale page instead of erroring.
...
- **`_includes/seo.html`**, included from `_layouts/default.html`'s `<head>`, emits a canonical link, a meta description, Open Graph tags, and Twitter Card tags. It reads only fields every page already has (`page.title`/`site.title`, `page.url`, the rendered `content`) or a generic alias set by the generator, the same "don't assume a site-specific field name" reasoning as the feed removal above:
  - `meta_description`/`og:description`/`twitter:description`: `page.description` if a content type happens to have one, else `content | strip_html | unescape_html | normalize_whitespace | truncate: 160` — works for any content type, since every generated page has rendered `content`. `unescape_html` (`contentful_filters.rb`) undoes one round of HTML-entity escaping that `strip_html` alone leaves behind (e.g. a Rich Text embedded entry's title, `CGI.escapeHTML`'d by `contentful_rich_text.rb`) before the final `| escape` re-escapes it for the meta tag — without it, an entity like `&quot;` would double-escape into `&amp;quot;`. `_plugins/contentful_listing_pages.rb`'s Utvalg/Produsenter listing/filter pages set an explicit `page.description` of their own for exactly this reason: their `content` is a rendered grid of product/manufacturer cards, not prose, so falling back to it would produce a meaningless truncated run-on instead of a sentence.
  - `og:image`/`twitter:image`: `page.social_image.url`, set by `EntriesGenerator#build_page` as `page.data[collection["image_field"] || "image"]` — a generic alias for whatever field holds a collection's social preview image, since content types name it differently (`coverImage`, `image`, `contentImage`, ...). Configure `image_field` per collection (see the `contentful_collections` table above) if it isn't literally `image`; harmless (just `nil`, same as any other absent field) if no such field exists. Falls back to `site.data.company.social_image` (`_data/company.yml`, `| absolute_url`'d) for a page with no image of its own (every listing/filter page, Om oss, Hjem) — unset today, so currently a no-op (`absolute_url(nil)` is `nil`). `twitter:card` switches between `summary_large_image` and `summary` based on whether either resolves to something.
